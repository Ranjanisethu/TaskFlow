...
function switchSettingTab(tabName) {
    ...
    if (tabName === 'security') renderLoginHistory();
}
...
function saveAppLockPin() {
    const pin = document.getElementById('setting-app-lock-pin').value;
    if (pin.length !== 4 || isNaN(pin)) {
        showNotification('PIN must be 4 digits', 'error');
        return;
    }
    const encoded = btoa(pin);
    Settings.save({ appLockPin: encoded });
    showNotification('App Lock PIN set successfully!');
}

function lockApp() {
    const s = Settings.load();
    if (!s.appLockPin) {
        showNotification('Set a PIN first!', 'error');
        return;
    }
...
function renderLoginHistory() {
    ...
    // Mock data for demo
    const history = [
        { device: navigator.platform + " (Current)", loc: "Unknown Location", time: "Active Now", status: "Active" },
        { device: "Mobile (iOS)", loc: "Region US", time: "2 hours ago", status: "Logged Out" },
        { device: "Desktop (Chrome)", loc: "Region EU", time: "Yesterday", status: "Success" }
    ];

    tbody.innerHTML = history.map(h => `
        <tr style="border-bottom:1px solid rgba(255,255,255,0.05);">
            <td style="padding:0.75rem;">${h.device}</td>
            <td style="padding:0.75rem;">${h.loc}</td>
            <td style="padding:0.75rem;">${h.time}</td>
            <td style="padding:0.75rem;">
                <span style="font-size:0.75rem; padding:0.25rem 0.5rem; border-radius:1rem; background:${h.status === 'Active' ? '#10b98120' : '#64748b20'}; color:${h.status === 'Active' ? '#10b981' : '#94a3b8'};">
                    ${h.status}
                </span>
            </td>
        </tr>
